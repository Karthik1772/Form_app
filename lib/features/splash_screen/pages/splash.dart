      ...
      child: BlocConsumer<OtpBloc, OtpState>(
        listener: (context, state) async {
          if (state is OtpSendSuccess) {
            ...
            final existing = await demographic.SheetsFlutter.checkIfEmailExists(
              _email.text.trim(),
            );
            ...
            if (existing) {
              CustomSnackbar.show(
                context: context,
                text: "Details already submitted from this email!",
                background: AppColors.white,
                textcolor: AppColors.orange,
                position: 70,
              );
                      ...
                      _otpSent
                          ...
                          : Padding(
                            ...
                            child: CustomButtons(
                              ...
                              onpressed: () async {
                                if (state is! OtpLoading) {
                                  final existing = await demographic
                                      .SheetsFlutter.checkIfEmailExists(
                                    _email.text.trim(),
                                  );
